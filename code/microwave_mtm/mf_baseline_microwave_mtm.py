...
Data layout MUST match the run-through version:
  data_dir/
    freqs.npy (or compatibility wavelengths.npy)
    idx_freqs.npy (or compatibility idx_wavelength.npy)
...
from mf_utils import (
    ...
    make_even_subsample_indices, pick_y_sub, upsample_y_sub_to_full, build_linear_interp_weights,
    # uncertainty
    propagate_subsample_var_to_full_y_var,
    ci_coverage_y, ci_width_y, calibrate_sigma_scale,
    # plot
    make_ci_bands_for_curve,
    # csv
    CsvLogger,
...
def crop_y_ri(y: np.ndarray, keep: np.ndarray, K_full: int) -> np.ndarray:
    y = np.asarray(y)
    if y.ndim != 2:
        raise ValueError(f"crop_y_ri expects 2D array, got {y.shape}")
    if y.shape[1] != 2 * K_full:
        raise ValueError(f"Expected y_dim=2*K_full={2*K_full}, got {y.shape[1]}")
    yr = y[:, :K_full][:, keep]
    yi = y[:, K_full:][:, keep]
    return np.concatenate([yr, yi], axis=1)
...
def load_frequency_axis(data_dir: Path) -> Tuple[np.ndarray, np.ndarray]:
    """Prefer microwave frequency-axis files, fallback to nanophotonic names."""
    candidates = [
        ('freqs.npy', 'idx_freqs.npy'),
        ('frequencies.npy', 'idx_frequencies.npy'),
        ('wavelengths.npy', 'idx_wavelength.npy'),
    ]
    for a_name, i_name in candidates:
        a_path = data_dir / a_name
        i_path = data_dir / i_name
        if a_path.exists() and i_path.exists():
            axis = np.load(a_path).astype(np.float32).reshape(-1)
            idx = np.load(i_path).astype(np.int64).reshape(-1)
            if axis.size >= 2 and np.any(np.diff(axis) < 0):
                order = np.argsort(axis)
                axis = axis[order]
                idx = idx[order]
            return axis, idx
    # mixed case: builder may still save wavelengths names only for compatibility
    a_path = data_dir / 'freqs.npy'
    if a_path.exists():
        axis = np.load(a_path).astype(np.float32).reshape(-1)
        idx = np.arange(axis.size, dtype=np.int64)
        return axis, idx
    raise FileNotFoundError(f'No frequency axis file found under {data_dir}. Tried freqs.npy/frequencies.npy/wavelengths.npy.')
DEFAULT_DATA_DIR = str(SCRIPT_DIR / '../../data/mf_dataset_mw_mtm/hf50_lfx10')
DEFAULT_OUT_DIR = str(SCRIPT_DIR / '../../result_out/mf_baseline_out_microwave_mtm')
...
def run_ours_via_a0(args: argparse.Namespace, out_dir: Path) -> Tuple[Optional[Dict[str, Any]], Optional[Path]]:
    ...
    if getattr(args, "freq_low", None) is not None:
        argv += ["--freq_low", str(float(args.freq_low))]
    if getattr(args, "freq_high", None) is not None:
        argv += ["--freq_high", str(float(args.freq_high))]
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--ours_train_script", type=str, default=str(Path(__file__).parent / "mf_train_microwave_mtm.py"),
    ...
    ap.add_argument("--freq_low", type=float, default=0, help="Crop frequency low (GHz)")
    ap.add_argument("--freq_high", type=float, default=30, help="Crop frequency high (GHz)")
    # dim reduce
...
def run_once(args: argparse.Namespace, seed: int) -> None:
    ...
    run_id = now_tag()
    run_name = build_run_name(args)  # 保留作日志/metadata，不再参与路径拼接
    _ensure_dir(out_dir)

    print("[BOOT] mf_baseline_hf_delta_ar1_aligned_lfprob_microwave_m013.py is running...")
    run_methods = parse_methods(getattr(args, 'methods', 'all'))
    print('[METHODS] enabled =', ','.join(sorted(run_methods)))
    # For delegated OURS
    ours_report = None
    ours_pred_root = None
    ours_from_delegate = False
    ...
    try:
            ...
            # wavelengths
            wl_full, idx_wl_full = load_frequency_axis(data_dir)
            K_full = int(wl_full.size)


            # optional crop band
            if (getattr(args, "freq_low", None) is not None) and (getattr(args, "freq_high", None) is not None):
                lo = float(args.freq_low)
                hi = float(args.freq_high)
                keep = np.where((wl_full >= lo) & (wl_full <= hi))[0]
                if keep.size <= 1:
                    raise ValueError(f"Empty/too small freq crop [{lo},{hi}]")

                wl = wl_full[keep].astype(np.float32)
                idx_wl = idx_wl_full[keep].astype(np.int64)

                y_hf_tr = crop_y_ri(y_hf_tr, keep, K_full)
                y_hf_va = crop_y_ri(y_hf_va, keep, K_full)
                y_hf_te = crop_y_ri(y_hf_te, keep, K_full)

                y_lfp_tr = crop_y_ri(y_lfp_tr, keep, K_full)
                y_lfp_va = crop_y_ri(y_lfp_va, keep, K_full)
                y_lfp_te = crop_y_ri(y_lfp_te, keep, K_full)

                y_lfu_tr = crop_y_ri(y_lfu_tr, keep, K_full)
                y_lfu_va = crop_y_ri(y_lfu_va, keep, K_full)
                y_lfu_te = crop_y_ri(y_lfu_te, keep, K_full)

                print(f"[CROP] freq_range=[{lo},{hi}] | K_full={K_full} -> K={int(wl.size)} | y_dim={2 * int(wl.size)}")
                dbg_print_wavelengths(wl_full, keep, wl)
            else:
                wl = wl_full.astype(np.float32)
                idx_wl = idx_wl_full.astype(np.int64)
            print(f"[DEBUG][CROP] hf/train {y_hf_tr.shape} hf/val {y_hf_va.shape} hf/test {y_hf_te.shape}")
            K = int(wl.size)
            print(f"[INFO] device={device}")
            print(f"[INFO] data_dir={data_dir}")
            print(f"[INFO] out_dir={out_dir}")
            print(f"[INFO] frequency K={K} | f=[{float(wl.min()):.6g},{float(wl.max()):.6g}] GHz")
            ...
            np.save(out_dir / "freqs.npy", wl.astype(np.float32))
            np.save(out_dir / "idx_freqs.npy", idx_wl.astype(np.int64))
            ...
            if "hf_only" in run_methods:
                ...
                svgp_hf = train_svgp_per_dim(
                    ...
                    csv_run_meta={"run_id": run_id, "run_name": run_name, "baseline": "microwave_m013_hf_delta_ar1", "seed": int(args.seed)},
                )
            ...
            if "ours" in run_methods and (ours_from_delegate or (svgp_ours is not None)):
            ...
            for j, i in enumerate(idx_cases):
                y_gt_plot = ri_to_mag_db(y_hf_te[[i]], K)[0]
                y_lf_plot = ri_to_mag_db(y_lfp_te[[i]], K)[0]

                y_hf_pred_plot = ri_to_mag_db(ypred_hf_te[[i]], K)[0]
                y_ar1_pred_plot = ri_to_mag_db(ypred_ar1_te[[i]], K)[0]
                y_ours_pred_plot = ri_to_mag_db(ypred_st_te[[i]], K)[0]

                std_hf_plot_raw = std_ri_to_mag_db_approx(ypred_hf_te[[i]], std_hf_te_raw[[i]], K)
                std_ar1_plot_raw = std_ri_to_mag_db_approx(ypred_ar1_te[[i]], std_ar1_te_raw[[i]], K)
                std_ours_plot_raw = std_ri_to_mag_db_approx(ypred_st_te[[i]], std_st_te_raw[[i]], K)

                std_hf_plot_cal = std_ri_to_mag_db_approx(ypred_hf_te[[i]], std_hf_te_cal[[i]], K)
                std_ar1_plot_cal = std_ri_to_mag_db_approx(ypred_ar1_te[[i]], std_ar1_te_cal[[i]], K)
                std_ours_plot_cal = std_ri_to_mag_db_approx(ypred_st_te[[i]], std_st_te_cal[[i]], K)

                # raw bands
                b = make_ci_bands_for_curve(y_hf_pred_plot, std_hf_plot_raw[0], ci_lvl)
                lo_hf_raw, hi_hf_raw = b["lo"], b["hi"]
                b = make_ci_bands_for_curve(y_ar1_pred_plot, std_ar1_plot_raw[0], ci_lvl)
                lo_ar1_raw, hi_ar1_raw = b["lo"], b["hi"]
                b = make_ci_bands_for_curve(y_ours_pred_plot, std_ours_plot_raw[0], ci_lvl)
                lo_ours_raw, hi_ours_raw = b["lo"], b["hi"]

                # cal bands
                if is_conformal:
                    # In conformal modes, *_cal are CI radii already: band = y ± radius (no extra z).
                    r = np.asarray(std_hf_plot_cal[0], dtype=np.float32)
                    lo_hf_cal, hi_hf_cal = y_hf_pred_plot - r, y_hf_pred_plot + r
                    r = np.asarray(std_ar1_plot_cal[0], dtype=np.float32)
                    lo_ar1_cal, hi_ar1_cal = y_ar1_pred_plot - r, y_ar1_pred_plot + r
                    r = np.asarray(std_ours_plot_cal[0], dtype=np.float32)
                    lo_ours_cal, hi_ours_cal = y_ours_pred_plot - r, y_ours_pred_plot + r
                else:

                    b = make_ci_bands_for_curve(y_hf_pred_plot, std_hf_plot_cal[0], ci_lvl)
                    lo_hf_cal, hi_hf_cal = b["lo"], b["hi"]
                    b = make_ci_bands_for_curve(y_ar1_pred_plot, std_ar1_plot_cal[0], ci_lvl)
                    lo_ar1_cal, hi_ar1_cal = b["lo"], b["hi"]
                    b = make_ci_bands_for_curve(y_ours_pred_plot, std_ours_plot_cal[0], ci_lvl)
                    lo_ours_cal, hi_ours_cal = b["lo"], b["hi"]

                # ---------- NEW: per-case RMSE (shared by all three plots) ----------
                rmse_hf = _rmse_1d(y_hf_pred_plot, y_gt_plot)
                rmse_ar1 = _rmse_1d(y_ar1_pred_plot, y_gt_plot)
                rmse_ours = _rmse_1d(y_ours_pred_plot, y_gt_plot)
                ...
                plot_case_5curves_spectrum_named(
                    ...
                    y_hf_gt=y_gt_plot,
                    y_lf=y_lf_plot,
                    y_pred_hf=y_hf_pred_plot,
                    y_pred_ar1=y_ar1_pred_plot,
                    y_pred_delta_svgp=y_ours_pred_plot,
                    title=f"TEST case{j:02d} (idx={i}) | noCI",
                    metrics_text=metrics_text_base,
                    show_gt_points=True,
                )
                ...
                # raw
                # =========================
                cov_hf_raw, wid_hf_raw = _cov_width_1d(y_gt_plot, lo_hf_raw, hi_hf_raw)
                cov_ar1_raw, wid_ar1_raw = _cov_width_1d(y_gt_plot, lo_ar1_raw, hi_ar1_raw)
                cov_ours_raw, wid_ours_raw = _cov_width_1d(y_gt_plot, lo_ours_raw, hi_ours_raw)
                ...
                # cal
                # =========================
                cov_hf_cal, wid_hf_cal = _cov_width_1d(y_gt_plot, lo_hf_cal, hi_hf_cal)
                cov_ar1_cal, wid_ar1_cal = _cov_width_1d(y_gt_plot, lo_ar1_cal, hi_ar1_cal)
                cov_ours_cal, wid_ours_cal = _cov_width_1d(y_gt_plot, lo_ours_cal, hi_ours_cal)
            ...
            report: Dict[str, Any] = {
                ...
                "out_dir": str(out_dir),
                "baseline": "microwave_m013: hf_only + ar1 + ours(delta-student)",
