...
# -*- coding: utf-8 -*-
"""
mf_train_fpca_sgp_delta_student_lfprob_microwave_m013.py  (Microwave M013, FPCA default)

Microwave M013 MF dataset: SVGP + FPCA/SubSampling + kernel ablations
...
from mf_utils import (
    ...
    # frequency + subsample
    load_frequencys, make_even_subsample_indices, pick_y_sub, upsample_y_sub_to_full, build_linear_interp_weights,
    # uncertainty
...
def load_axis_microwave_compat(data_dir: Path):
    """Prefer freqs.npy for microwave datasets, fall back to frequencys.npy for compatibility."""
    p_freq = data_dir / "freqs.npy"
    if p_freq.exists():
        axis = np.load(p_freq).astype(np.float32).reshape(-1)
        idx = np.arange(axis.size, dtype=np.int64)
        return axis, idx, "freq"
    # fallback to mf_utils-compatible loader
    axis, idx = load_frequencys(data_dir)
    return axis.astype(np.float32), idx.astype(np.int64), "frequency"

SCRIPT_DIR = Path(__file__).resolve().parent
DEFAULT_DATA_DIR = str(SCRIPT_DIR / "../../data/mf_dataset_mw_mtm/hf50_lfx10")
# DEFAULT_DATA_DIR = str(SCRIPT_DIR / "./absorbance_database_500h_10t_1.0")
DEFAULT_OUT_DIR = str(SCRIPT_DIR / "../../result_out/mf_out_microwave_mtm")
# =========================
...
def dbg_print_axis(axis_full: np.ndarray, idx_keep: Optional[np.ndarray], axis_used: np.ndarray) -> None:
    h_full, t_full = _dbg_head_tail(axis_full, 5)
    h_used, t_used = _dbg_head_tail(axis_used, 5)
    print(f"[DEBUG][AXIS] axis_full head5={h_full} tail5={t_full}")
    print(f"[DEBUG][AXIS] axis_used head5={h_used} tail5={t_used}")
    if idx_keep is not None:
        axis_check = axis_full[idx_keep]
        assert axis_check.shape == axis_used.shape
        assert np.allclose(axis_check.astype(np.float32), axis_used.astype(np.float32)), "axis_used != axis_full[idx_keep]"
...
class FPCA:
    """
    FPCA for discretized curves y(frequency) sampled on a fixed grid.
...
def infer_y_layout(y_dim: int, K_axis: int) -> str:
    y_dim = int(y_dim)
    K_axis = int(K_axis)
    if y_dim == K_axis:
        return "single"
    if y_dim == 2 * K_axis:
        return "ri"
    raise ValueError(f"Unsupported y layout: y_dim={y_dim}, K_axis={K_axis}")


def axis_slice_y(y: np.ndarray, idx_keep: np.ndarray, K_axis: int) -> np.ndarray:
    y = np.asarray(y)
    idx_keep = np.asarray(idx_keep, dtype=np.int64).reshape(-1)
    mode = infer_y_layout(y.shape[1], K_axis)
    if mode == "single":
        return y[:, idx_keep].astype(np.float32, copy=False)
    yr = y[:, :K_axis][:, idx_keep]
    yi = y[:, K_axis:][:, idx_keep]
    return np.concatenate([yr, yi], axis=1).astype(np.float32, copy=False)


def upsample_axisaware(y_sub: np.ndarray, idx_k: np.ndarray, axis_full: np.ndarray, K_axis: int) -> np.ndarray:
    y_sub = np.asarray(y_sub, dtype=np.float32)
    mode = infer_y_layout(y_sub.shape[1], len(idx_k))
    if mode == "single":
        return upsample_y_sub_to_full(y_sub, idx_k=idx_k, wl_full=axis_full).astype(np.float32)
    Ks = int(len(idx_k))
    yr = upsample_y_sub_to_full(y_sub[:, :Ks], idx_k=idx_k, wl_full=axis_full).astype(np.float32)
    yi = upsample_y_sub_to_full(y_sub[:, Ks:], idx_k=idx_k, wl_full=axis_full).astype(np.float32)
    return np.concatenate([yr, yi], axis=1).astype(np.float32)


def ri_to_mag_db(y: np.ndarray, K_axis: int, eps: float = 1e-12) -> np.ndarray:
    ...
    yr = y[:, :K_axis]
    yi = y[:, K_axis:]
    mag = np.sqrt(np.maximum(yr * yr + yi * yi, 0.0))
    mag = np.maximum(mag, float(eps))
    return (20.0 * np.log10(mag)).astype(np.float32)


def std_ri_to_mag_db_approx(y_mean: np.ndarray, std_y: np.ndarray, K_axis: int, eps: float = 1e-12) -> Optional[np.ndarray]:
    """Approximate std in dB-magnitude from independent std in real/imag via delta method.
    Returns None for unsupported shapes.
    """
    y_mean = np.asarray(y_mean, dtype=np.float32)
    std_y = np.asarray(std_y, dtype=np.float32)
    try:
        mode = infer_y_layout(y_mean.shape[1], K_axis)
    except Exception:
        return None
    if mode == "single":
        return std_y.astype(np.float32, copy=False)
    yr = y_mean[:, :K_axis]
    yi = y_mean[:, K_axis:]
    sr = std_y[:, :K_axis]
    si = std_y[:, K_axis:]
    mag2 = np.maximum(yr * yr + yi * yi, float(eps) ** 2)
    var_mag = (yr * yr / mag2) * (sr * sr) + (yi * yi / mag2) * (si * si)
    mag = np.sqrt(mag2)
    coeff = 20.0 / np.log(10.0)
    std_db = coeff * np.sqrt(np.maximum(var_mag, 0.0)) / np.maximum(mag, float(eps))
    return std_db.astype(np.float32)
...
def build_run_name(args: argparse.Namespace) -> str:
    ...
    fall back to a stable hashed short name: mw0 + 7 hex chars.
    """
    import hashlib
    ...
    if str(args.dim_reduce).lower().strip() == "subsample":
        ...
        # default FPCA baseline is: fpca_dim=0, var_ratio=0.999, max_dim=50
...
def main():
    print("[BOOT] mf_train_fpca_sgp_delta_student_lfprob_microwave_m013.py is running...")

    ap = argparse.ArgumentParser()

    # --- paths
    ap.add_argument("--data_dir", type=str, default=DEFAULT_DATA_DIR,
                    help="Dataset root (contains freqs.npy / frequencys.npy and hf/ lf_paired/ lf_unpaired/).")
    ...
    # --- frequency crop (optional)
    ap.add_argument("--freq_low", type=float, default=0.0,
                    help="If set together with --freq_high, crop frequency points to the inclusive range [freq_low, freq_high] (GHz).")
    ap.add_argument("--freq_high", type=float, default=30.0,
                    help="Upper bound for frequency crop (GHz). Must be used together with --freq_low.")
    ap.add_argument("--freq_crop_strict", type=int, default=1, choices=[0, 1],
    ...
    # -------------------------
    # Load frequency axis + data splits
    # -------------------------
    axis_full, idx_axis_full, axis_name = load_axis_microwave_compat(data_dir)
    K_full = int(axis_full.size)

    # Decide frequency crop indices early (apply after y is loaded)
    idx_keep_axis = None
    if (args.freq_low is not None) or (args.freq_high is not None):
        if (args.freq_low is None) or (args.freq_high is None):
            raise SystemExit("--freq_low and --freq_high must be provided together.")
        lo = float(min(args.freq_low, args.freq_high))
        hi = float(max(args.freq_low, args.freq_high))
        mask = (axis_full >= lo) & (axis_full <= hi)
        idx_keep_axis = np.where(mask)[0].astype(np.int64)
        if idx_keep_axis.size == 0:
            if bool(int(args.freq_crop_strict)):
                raise SystemExit(f"Frequency crop [{lo},{hi}] GHz keeps 0 points (K_full={K_full}).")
            idx_keep_axis = None
    ...
    # --- apply optional frequency crop
    if idx_keep_axis is not None:
        def _crop_y(y: np.ndarray, name: str) -> np.ndarray:
            ...
            if y.shape[1] == K_full:
                return y[:, idx_keep_axis].astype(np.float32, copy=False)
            if y.shape[1] == 2 * K_full:
                y_r = y[:, :K_full][:, idx_keep_axis]
                y_i = y[:, K_full:][:, idx_keep_axis]
                return np.concatenate([y_r, y_i], axis=1).astype(np.float32, copy=False)
            raise SystemExit(
                f"y second-dim mismatch for {name}: y.shape[1]={y.shape[1]} "
                f"vs expected K_full={K_full} or 2*K_full={2*K_full}"
            )
        ...
        axis = axis_full[idx_keep_axis].astype(np.float32)
        idx_axis = idx_axis_full[idx_keep_axis].astype(np.int64)
        K = int(axis.size)

        lo = float(min(args.freq_low, args.freq_high))
        hi = float(max(args.freq_low, args.freq_high))
        print(f"[CROP] freq_range=[{lo},{hi}] | K_full={K_full} -> K={K}")
        dbg_print_axis(axis_full=axis_full, idx_keep=idx_keep_axis, axis_used=axis)
    else:
        axis = axis_full.astype(np.float32)
        idx_axis = idx_axis_full.astype(np.int64)
        K = int(axis.size)
    ...
    print(f"[INFO] axis_name=frequency K={K} | freq_range=[{float(axis.min()):.6g},{float(axis.max()):.6g}] GHz")
    # hard asserts: HF/LF y must match frequency length after crop/subsample
    for _name, _y in [
        ...
        ydim = int(_y.shape[1])
        if ydim not in (K, 2 * K):
            raise AssertionError(f"{_name} y.shape[1]={ydim} not in {{K={K}, 2*K={2*K}}}")
    # HF/LF use identical frequencys (single source of truth)
    assert axis.shape[0] == K
    ...
    print(f"[INFO] y_dim(train HF) = {y_hf_tr.shape[1]} | expected layouts: K = {K}, 2*K = {2*K}")
    ...
    config = {
        ...
        "K": K, "xdim": xdim, "axis_name": axis_name,
    }
    with open(out_dir / "config.json", "w", encoding="utf-8") as f:
        json.dump(config, f, ensure_ascii=False, indent=2)
    np.save(out_dir / "freqs_used.npy", axis.astype(np.float32))
    np.save(out_dir / "wavelengths.npy", axis.astype(np.float32))
    np.save(out_dir / "idx_wavelength.npy", idx_axis.astype(np.int64))

    # -------------------------
    # Stage-I: student (x -> LF oracle y)
    # -------------------------
    print(f"[STAGE-I] Train student (feature+head): x -> LF oracle y (y_dim={int(y_lfp_tr.shape[1])})")
    ...
    if skip_student:
        ...
        student_y_dim = int(y_stu_train.shape[1])
        out_dim_student = (2 * student_y_dim) if int(getattr(args, "lf_prob", 0)) == 1 else student_y_dim
        student = FeatureMLP(
            in_dim=xdim,
        ...
        if int(getattr(args, "lf_prob", 0)) == 1:
            yhat_mix_tr_s, yvar_mix_tr_s, _ = mlp_predict_mu_logvar_and_features(
                student, X_lf_tr_all_s, device=device, batch_size=4096, K=student_y_dim,
                logvar_clip=float(getattr(args, "lf_logvar_clip", 10.0)),
            )
            yhat_mix_va_s, yvar_mix_va_s, _ = mlp_predict_mu_logvar_and_features(
                student, X_lf_va_all_s, device=device, batch_size=4096, K=student_y_dim,
                logvar_clip=float(getattr(args, "lf_logvar_clip", 10.0)),
            )
            yhat_mix_te_s, yvar_mix_te_s, _ = mlp_predict_mu_logvar_and_features(
                student, X_lf_te_all_s, device=device, batch_size=4096, K=student_y_dim,
                logvar_clip=float(getattr(args, "lf_logvar_clip", 10.0)),
            )
        else:
        ...
        if int(getattr(args, "lf_prob", 0)) == 1:
            yhat_lf_tr_s, yvar_lf_tr_s, feat_tr = mlp_predict_mu_logvar_and_features(
                student, x_hf_tr_s_for_student, device=device, batch_size=4096, K=student_y_dim,
                logvar_clip=float(getattr(args, "lf_logvar_clip", 10.0)),
            )
            yhat_lf_va_s, yvar_lf_va_s, feat_va = mlp_predict_mu_logvar_and_features(
                student, x_hf_va_s_for_student, device=device, batch_size=4096, K=student_y_dim,
                logvar_clip=float(getattr(args, "lf_logvar_clip", 10.0)),
            )
            yhat_lf_te_s, yvar_lf_te_s, feat_te = mlp_predict_mu_logvar_and_features(
                student, x_hf_te_s_for_student, device=device, batch_size=4096, K=student_y_dim,
                logvar_clip=float(getattr(args, "lf_logvar_clip", 10.0)),
            )
        else:
    ...
    if dim_reduce == "fpca":
        ...
        np.save(out_dir / "freqs_subsample.npy", axis_sub.astype(np.float32))
        np.save(out_dir / "wavelengths_subsample.npy", axis_sub.astype(np.float32))

        W_full_from_sub = build_linear_interp_weights(x_full=axis, x_sub=axis_sub)
        save_pickle(out_dir / "interp_weights_full_from_sub.pkl", W_full_from_sub)
        print(f"[SUB] Ks={Ks_int} | axis_sub=[{float(axis_sub.min()):.6g},{float(axis_sub.max()):.6g}]")
        print(f"[DEBUG][SUB] K_band={K} -> Ks={Ks_int} | idx_k={idx_k.tolist()}")
        h_sub, t_sub = _dbg_head_tail(axis_sub, 5)
        print(f"[DEBUG][SUB] axis_sub head5={h_sub} tail5={t_sub}")

        y_hf_tr_sub = axis_slice_y(y_hf_tr, idx_k, K)
        y_hf_va_sub = axis_slice_y(y_hf_va, idx_k, K)
        y_hf_te_sub = axis_slice_y(y_hf_te, idx_k, K)
        ...
        z_lf_tr_repr = axis_slice_y(y_lf_tr, idx_k, K)
        z_lf_va_repr = axis_slice_y(y_lf_va, idx_k, K)
        z_lf_te_repr = axis_slice_y(y_lf_te, idx_k, K)

        z_hat_tr_repr = axis_slice_y(yhat_lf_tr, idx_k, K)
        z_hat_va_repr = axis_slice_y(yhat_lf_va, idx_k, K)
        z_hat_te_repr = axis_slice_y(yhat_lf_te, idx_k, K)

        def inv_target_to_y_full(mu_target_scaled: np.ndarray) -> np.ndarray:
            assert idx_k is not None
            y_sub = scaler_hf_target.inverse_transform(mu_target_scaled.astype(np.float32)).astype(np.float32)
            return upsample_axisaware(y_sub, idx_k=idx_k, axis_full=axis, K_axis=K)
...
# standardize u separately for oracle/student (keeps previous behavior)
        ...
        if do_mc:
            ...
            def _lf_repr_from_yraw(y_raw: np.ndarray) -> np.ndarray:
                if dim_reduce == "fpca":
                    ...
                    assert idx_k is not None
                    return axis_slice_y(y_raw.astype(np.float32), idx_k, K).astype(np.float32)
    ...
    if int(getattr(args, "save_pred_arrays", 0)) == 1:
        ...
        # axis
        np.save(pred_root / "axis.npy", axis.astype(np.float32))
    ...
    np.save(rmse_curves_dir / "axis.npy", axis.astype(np.float32))
    ...
    def plot_split(
        ...
        for j, ii in enumerate(idx_pick.tolist()):
            ...
            y_hf_plot = ri_to_mag_db(y_hf_gt[[ii]], K)[0]
            y_lf_plot = ri_to_mag_db(y_lf[[ii]], K)[0]
            y_st_plot = ri_to_mag_db(y_pred_st[[ii]], K)[0]
            ...
            if int(args.plot_ci) == 1:
                std_st_plot_raw = std_ri_to_mag_db_approx(y_pred_st[[ii]], std_st_raw[[ii]], K)
                std_st_plot_cal = std_ri_to_mag_db_approx(y_pred_st[[ii]], std_st_cal[[ii]], K)
                mf_band_raw = None if std_st_plot_raw is None else make_ci_bands_for_curve(y_st_plot, std_st_plot_raw[0], ci_lvl)
