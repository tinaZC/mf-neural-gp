...
# Timing helpers
# ----------------------------
def _sync_cuda_if_needed(device=None) -> None:
    """Synchronize CUDA before/after timing so wall-clock measurements are stable."""
    try:
...
def _toc(t0: float, device=None) -> float:
    _sync_cuda_if_needed(device)
    return float(time.perf_counter() - float(t0))


def _flatten_timing_for_csv(timing: Dict[str, Any], prefix: str = "time") -> Dict[str, Any]:
    """Flatten nested timing dict into CSV-safe scalar columns."""
    out: Dict[str, Any] = {}

    def _walk(obj: Any, key: str) -> None:
        if isinstance(obj, dict):
            for kk, vv in obj.items():
                _walk(vv, f"{key}_{kk}" if key else str(kk))
...
def _jsonable(obj: Any):
    """Convert numpy / torch scalar-like objects to JSON-safe python objects."""
    if isinstance(obj, dict):
...
def save_timing_json(out_dir: Path, timing: Dict[str, Any], filename: str = "timing.json") -> Path:
    """Save timing information separately from report.json/results.csv."""
    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    p = out_dir / filename
    with open(p, "w", encoding="utf-8") as f:
        json.dump(_jsonable(timing), f, ensure_ascii=False, indent=2)
    print(f"[TIMING] saved: {p}")
    return p
...
try:
    ...
    from .mf_train import FeatureMLP
except ImportError:
    from mf_utils import (
        set_seed, now_tag, safe_tag, save_pickle, rmse, r2_score, gaussian_nll, gaussian_nlpd, z_from_ci_level, pca_recon_rmse,
        load_wavelengths, make_even_subsample_indices, pick_y_sub, upsample_y_sub_to_full, build_linear_interp_weights,
...
DEFAULT_DATA_DIR = ""
DEFAULT_OUT_DIR = ""
DEFAULT_RUN_PREFIX = "mb"
DEFAULT_OURS_TRAIN_SCRIPT = ""
...
class BaselineDefaults:
    ...
    ours_train_script: str = DEFAULT_OURS_TRAIN_SCRIPT
...
def build_run_name(args: argparse.Namespace) -> str:
    """
    Short run-name rule:
      - base comes from --run_prefix (default: mb)
      - only append short codes for NON-default settings
      - if name grows beyond 10 chars, fall back to <base> + 7-char hash
    """
    base = safe_tag(str(getattr(args, "run_prefix", "mb")).strip() or "mb")
    tokens = []
    ...
    if int(args.svgp_steps) != 2000:
        ...
        add(f"t{steps // 1000}k" if steps % 1000 == 0 else f"t{steps}")
...
def run_ours_via_a0(args: argparse.Namespace, out_dir: Path) -> Tuple[Optional[Dict[str, Any]], Optional[Path]]:
    ...
    if int(getattr(args, "delegate_ours_to_train", 1)) != 1:
        return None, None
    script = Path(getattr(args, "ours_train_script") or (Path(__file__).resolve().parent / "mf_train.py")).expanduser().resolve()
    ...
    argv = [str(script),
            ...
            "--run_prefix", str(getattr(args, "run_prefix", "mf")),
            "--no_subdir", "1",
...
def parse_args(argv: Optional[Sequence[str]] = None, defaults: Optional[BaselineDefaults] = None) -> argparse.Namespace:
    d = defaults or BaselineDefaults()
    ap = argparse.ArgumentParser()

    # paths
    ap.add_argument("--data_dir", type=str, default=d.data_dir, help="MF dataset root")
    ap.add_argument("--out_dir", type=str, default=d.out_dir, help="Output root directory")
    ap.add_argument("--run_prefix", type=str, default=d.run_prefix, help="Short run prefix used by auto run-name generation")
    ...
    ap.add_argument("--ours_train_script", type=str, default=(d.ours_train_script or str(Path(__file__).resolve().parent / "mf_train.py")),
...
def run_once(args: argparse.Namespace, seed: int) -> None:
    ...
    timing: Dict[str, Any] = {
        ...
        "n_hf_val": None,
        "n_hf_test": None,
        "n_lf_train_total": None,
        "n_lf_val_total": None,
        "n_lf_test_total": None,
        "stage1_train_s": None,
        "stage1_predict_s": None,
        "stage2_train_s": {
            "hf_only": None,
            "ar1_lf_svgp": None,
            "ar1_delta_svgp": None,
            "ar1_total": None,
            "ours_infile": None,
            "ours_delegate_total": None,
        },
        "inference_s": {
            "hf_only_val_test": None,
            "ar1_val_test": None,
            "ours_val_test": None,
        },
        "ours_delegate_timing": None,
        "ours_delegate_timing_path": None,
        "total_model_wall_s": None,
    }
    _t_total_model = _tic(device)
    try:
            ...
            if "ours" in run_methods:
                ...
                timing["stage1_train_s"] = _toc(_t_stage1_train, device)
                ...
                timing["stage1_predict_s"] = _toc(_t_stage1_predict, device)
            ...
            if "hf_only" in run_methods:
                ...
                timing["stage2_train_s"]["hf_only"] = _toc(_t_stage2_hf, device)
            ...
            if "ar1" in run_methods:
                ...
                timing["stage2_train_s"]["ar1_lf_svgp"] = _toc(_t_stage2_ar1_lf, device)
                ...
                _t_stage2_ar1_delta = _tic(device)
                ...
                timing["stage2_train_s"]["ar1_delta_svgp"] = _toc(_t_stage2_ar1_delta, device)
                timing["stage2_train_s"]["ar1_total"] = (
                    (timing["stage2_train_s"].get("ar1_lf_svgp") or 0.0)
                    + (timing["stage2_train_s"].get("ar1_delta_svgp") or 0.0)
                )
            ...
            if "ours" in run_methods:
                if int(getattr(args, "delegate_ours_to_train", 1)) == 1:
                    ...
                    _t_stage2_ours_delegate = _tic(device)
                    ours_report, ours_pred_root = run_ours_via_a0(args, out_dir)
                    timing["stage2_train_s"]["ours_delegate_total"] = _toc(_t_stage2_ours_delegate, device)
                    if isinstance(ours_report, dict) and "timing" in ours_report:
                        timing["ours_delegate_timing"] = ours_report.get("timing")
                    else:
                        ours_timing_path = Path(out_dir) / "ours" / "timing.json"
                        if ours_timing_path.exists():
                            with open(ours_timing_path, "r", encoding="utf-8") as _f:
                                timing["ours_delegate_timing"] = json.load(_f)
                            timing["ours_delegate_timing_path"] = str(ours_timing_path)
                    svgp_ours = None
                    Y_tr_ours = None
                else:
                    # ---- delta_svGP (in-file)
                    ...
                    _t_stage2_ours_infile = _tic(device)
                    ...
                    timing["stage2_train_s"]["ours_infile"] = _toc(_t_stage2_ours_infile, device)
            ...
            if svgp_hf is None:
                ...
                timing["inference_s"]["hf_only_val_test"] = _toc(_t_infer_hf, device)
            ...
            if (svgp_lf is None) or (svgp_delta is None) or (sx_lf is None) or (rho_ar1 is None):
                ...
                timing["inference_s"]["ar1_val_test"] = _toc(_t_infer_ar1, device)

            # Ours
            if ours_pred_root is not None:
                _t_infer_ours = _tic(device)
                ...
                timing["inference_s"]["ours_val_test"] = _toc(_t_infer_ours, device)
            ...
            timing["n_hf_train"] = int(x_hf_tr.shape[0])
            timing["n_hf_val"] = int(x_hf_va.shape[0])
            timing["n_hf_test"] = int(x_hf_te.shape[0])
            timing["n_lf_train_total"] = int(x_lfp_tr.shape[0] + x_lfu_tr.shape[0])
            timing["n_lf_val_total"] = int(x_lfp_va.shape[0] + x_lfu_va.shape[0])
            timing["n_lf_test_total"] = int(x_lfp_te.shape[0] + x_lfu_te.shape[0])
            timing["total_model_wall_s"] = _toc(_t_total_model, device)

            save_timing_json(out_dir, timing)
...
def main(argv: Optional[Sequence[str]] = None, defaults: Optional[BaselineDefaults] = None) -> None:
