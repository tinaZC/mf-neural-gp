...
DEFAULT_DATA_DIR = str(SCRIPT_DIR / "../../data/mf_sweep_datasets_nano_ab/hf50_lfx10")
DEFAULT_OUT_DIR = str(SCRIPT_DIR / "../../result_out/mf_baseline_nano_ab_out")
...
def build_run_name(args: argparse.Namespace) -> str:
    """
    Short run-name policy for baseline AB:
      - default baseline: ba0
      - only append short codes for non-default settings
      - if the name would exceed 10 chars, fall back to ba0 + 7-char hash
    Full configuration is already saved in config.json / report.json / results.csv,
    so the directory name only needs to be a compact index.
    """
    base = "ba0"
    toks: List[str] = []

    # effective defaults for this baseline
    if str(args.dim_reduce).lower() == "fpca":
        if int(args.fpca_dim) > 0:
            toks.append(f"f{int(args.fpca_dim)}")
        else:
            if abs(float(args.fpca_var_ratio) - 0.999) > 1e-12:
                v = str(args.fpca_var_ratio).replace("0.", "").replace(".", "p")
                toks.append(f"v{v}")
            if int(args.fpca_max_dim) != 64:
                toks.append(f"m{int(args.fpca_max_dim)}")
    else:
        toks.append(f"s{int(args.subsample_K)}")

    if str(args.mf_u_mode) != "xlf":
        mode_map = {"lf": "l", "xlf": "x", "x": "x0"}
        toks.append(mode_map.get(str(args.mf_u_mode), safe_tag(str(args.mf_u_mode))[:2]))
    ...
    if str(args.kernel_struct) != "full":
        ks_map = {"full": "kf", "block": "kb", "xlf_block": "kx"}
        toks.append(ks_map.get(str(args.kernel_struct), f"k{safe_tag(str(args.kernel_struct))[:2]}"))

    if str(args.kernel) != "matern":
        toks.append("rb")
    else:
        if abs(float(args.matern_nu) - 2.5) > 1e-12:
            nu_map = {0.5: "n05", 1.5: "n15", 2.5: "n25"}
            toks.append(nu_map.get(float(args.matern_nu), f"n{str(args.matern_nu).replace('.', 'p')}"))

    if int(args.gp_ard) != 1:
        toks.append("a0")

    if int(args.svgp_M) != 64:
        toks.append(f"M{int(args.svgp_M)}")

    if int(args.svgp_steps) != 2000:
        steps = int(args.svgp_steps)
        if steps % 1000 == 0:
            toks.append(f"t{steps // 1000}k")
        else:
            toks.append(f"t{steps}")

    if int(args.seed) != 42:
        toks.append(f"r{int(args.seed)}")

    if abs(float(args.ci_level) - 0.95) > 1e-12:
        c = str(args.ci_level).replace("0.", "").replace(".", "p")
        toks.append(f"c{c}")

    # main() forces ci_calibrate=0 for this baseline; only record non-default effective value
    if int(args.ci_calibrate) != 0:
        toks.append(f"q{int(args.ci_calibrate)}")
    ...
    h = hashlib.md5(json.dumps(payload, sort_keys=True).encode("utf-8")).hexdigest()[:7]
    return f"{base}{h}"
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--ours_train_script", type=str, default=str(Path(__file__).parent / "mf_train_ab.py"),
